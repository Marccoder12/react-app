...
export function SignUpModalForm() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (password !== confirmPassword) {
      //show Toast message
      addToast("error", "Paswwords do not match!");
    ...
    if (!email?.trim() || !password) {
      //show Toast message
      addToast("error", "Email and paswword are required!");
    ...
    try {
      const { data, error } = await supabase.auth.signUp({
        ...
        options: {
          ...
          data: {
            ...
            display_name: `${firstName.trim().toLowerCase()} ${lastName.trim().toLowerCase()}`,
          },
        },
      });
      ...
      addToast("success", "SignUp was successful!");
